...
void setup() 
{
    ...
    if ((rmt_send = rmtInit(PIN_OUT3, true, RMT_MEM_64)) == NULL)
    {
    ...
    float realTick = rmtSetTick(rmt_send, TICK); // 1250/25 = 50
...
void setLeds(uint32_t t_ms, uint32_t (*funcp)(uint32_t, uint32_t)) {
  uint32_t i = 0;
...
void sequence(uint32_t cycles_ms, uint32_t (*funcp)(uint32_t, uint32_t)) {
...
void loop() 
{
  sequence(5000, &rainbowSnakes);
  sequence(1000, &lampki);
  sequence(500, &eu);
  sequence(500, &narodowe);
  sequence(1000, &runningRainbow); 
}
